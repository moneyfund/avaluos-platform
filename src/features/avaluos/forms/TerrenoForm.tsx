...
export default function TerrenoForm({ value, onChange, onSubmit, loading, showSubmit = true }) {
  ...
  const setArea = (nextArea, nextUnit = unidadArea) => {
    ...
    onChange('areaOriginal', area); onChange('unidadArea', nextUnit); onChange('areaM2Convertida', converted); onChange('areaTerreno', converted); onChange('precioBaseFueEditado', false);
  };

  return <div className='text-slate-200'>
    <InformeGeneralSection value={value} onChange={onChange} />
    <div className='mb-5 rounded-2xl border border-amber-500/20 bg-amber-500/10 p-4'><h2 className='text-lg font-semibold text-amber-100'>Ficha técnica profesional de terreno</h2><p className='mt-1 text-sm text-amber-50/80'>Campos cerrados, cuantificables y conectados al motor técnico para solares urbanos, áreas semiurbanas y terrenos rurales de Matagalpa y Estelí.</p></div>
    ...
    <Section title='Clasificación territorial y suelo'>
      ...
      {num('Frente (m)', 'frenteTerreno', value, onChange)} {num('Fondo (m)', 'fondoTerreno', value, onChange)}
    </Section>
    ...
    <Section title='Entorno, uso y desarrollo'>
      ...
      {field('Restricciones legales', 'restriccionesLegales', value, onChange)} {field('Uso permitido', 'usoPermitido', value, onChange)} {field('Observaciones legales', 'observacionesLegales', value, onChange)}
